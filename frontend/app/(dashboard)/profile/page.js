...
const inputCls = 'w-full px-4 py-3.5 rounded-xl text-sm outline-none transition-all border tf-input-field';
...
export default function ProfilePage() {
  ...
  const card = 'rounded-2xl border tf-border p-5 sm:p-6';
    ...
    <div className="max-w-2xl mx-auto space-y-5">
      ...
      <div className={`${card} flex flex-col sm:flex-row items-start sm:items-center gap-5`}
        ...
        <div className="relative flex-shrink-0">
          <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-violet-600 to-cyan-500 flex items-center justify-center text-white text-3xl font-bold shadow-violet-md">
            {user?.name?.[0]?.toUpperCase()}
          </div>
          <div className="absolute -bottom-1 -right-1 w-6 h-6 rounded-lg bg-emerald-500 border-2 flex items-center justify-center"
            style={{ borderColor: 'var(--bg-surface)' }}>
            <div className="w-2 h-2 rounded-full bg-white" />
          </div>
        </div>
        <div>
          <h2 className="text-xl font-bold tf-text-1 leading-none">{user?.name}</h2>
          <p className="tf-text-2 text-sm mt-1">{user?.email}</p>
        ...
        <form onSubmit={handleProfileSave} className={`${card} space-y-4 animate-fade-up tf-surface`}>
          ...
          <div className="space-y-1.5">
            ...
            <input type="email" value={user?.email} disabled
              className="w-full px-4 py-3.5 rounded-xl tf-text-3 text-sm border tf-border cursor-not-allowed"
          ...
          <div className="space-y-1.5">
            ...
            <textarea value={profile.bio} onChange={e => setProfile({...profile,bio:e.target.value})}
              rows={3} maxLength={200} placeholder="Tell something about yourself…"
          ...
          <button type="submit" disabled={loadingP}
            ...
            {loadingP ? <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <Save className="w-4 h-4" />}
            Save Changes
          </button>
        </form>
      )}
      ...
      {tab === 'password' && (
        <form onSubmit={handlePasswordSave} className={`${card} space-y-4 animate-fade-up tf-surface`}>
          ...
          <button type="submit" disabled={loadingPw}
            className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-white text-sm font-semibold transition-all hover:-translate-y-0.5 active:translate-y-0 disabled:opacity-60 shadow-violet-sm hover:shadow-violet-md"
            style={{ background:'linear-gradient(135deg,#7c3aed,#06b6d4)' }}>
            {loadingPw ? <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <Lock className="w-4 h-4" />}
            Update Password
          </button>
        </form>
      )}
